...
for OSVER in `echo %{os_versions} | tr , ' '`; do
    install -d -m 0755 %{buildroot}%{repo_dir}/opensuse"${OSVER}"
    install -d -m 0755 %{buildroot}%{repo_dir}/opensuse"${OSVER}"/{i{3,5}86,x86_64,noarch,src,repodata,cache}
    install -m 0644 repo/"${OSVER}"/%{org_id}.repo %{buildroot}%{repo_dir}/opensuse"${OSVER}"/
done
